...
@Component({
  selector: 'app-architecture-representatives',
  standalone: true,
  templateUrl: './architecture-representatives.component.html',
  styleUrl: './architecture-representatives.component.css',
  imports: [
    CmsTopbarComponent,
    TopbarComponent,
    NgxSpinnerModule,
...
export class ArchitectureRepresentativesComponent {
  isLoading = true;
  imageUrl = environment.image_url_server;
  architectureRepresentLayouts!: any;
  others: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  arRepresentDescription: any;
  hasChanges: boolean = false;
    ...
    fetchData() {
      this.service.getar_Representatives().subscribe(
        (data) => {
          this.architectureRepresentLayouts = data.data;
    ...
    onLayoutSelected(layoutId: any) {
      ...
      if (selectedLayout) {
        if (layoutId === '32') {
          this.architectureRepresentLayouts.push({
            id: ``,
            title: '',
            description: '',
        ...
        if (this.architectureRepresentLayouts.length > 0) {
    ...
    onLayoutReplaceSelected(event: any) {
      if (this.selectedLayoutToReplace) {
        ...
        if (selectedLayout.layout_id === '32') {
          this.architectureRepresentLayouts[index] = {
            id: id,
            title: '',
            description: '',
            layoutid: selectedLayout.layout_id,
    ...
    onFilesSelectedLayout(
      ...
      reader.onload = () => {
        ...
        this.architectureRepresentLayouts.forEach((project: any, projectIndex: any) => {
    ...
    openEditLayoutDialog(index?: any) {
      const id = this.architectureRepresentLayouts[index].id;
    ...
    changeTitle(event: any) {
      this.hasChanges = true;
      this.architectureRepresentLayouts[1].title = event;
    }
  
    changeDescription(event: any) {
      this.hasChanges = true;
      this.architectureRepresentLayouts[1].description = event.replace(/Â/g, '');
    }
    ...
    onLayoutDelete() {
      ...
      this.spinner.show();
      this.architectureRepresentLayouts.splice(index, 1);
      this.hasChanges = true;
      this.spinner.hide();
      this.isVisible = false;
    }
    ...
    onPublish() {
      ...
      this.spinner.show();
      this.service
        .postar_RepresentativesPageData(this.architectureRepresentLayouts)
        .subscribe((data) => {
          if (data.message === 'success') {
            ...
            this.msgService.errorToaster(
              'Please add arrepresentatives studio image or name.'
            );
            this.fetchData();
            this.spinner.hide();
          }
