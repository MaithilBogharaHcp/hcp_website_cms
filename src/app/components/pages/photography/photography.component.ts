...
export class PhotographyComponent {
  ...
  photographyLayouts!: any;
  others: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  photographyDescription: any;
  ...
  fetchData() {
   this.service.getPhotography().subscribe(
      (data) => {
        this.photographyLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      ...
      if (this.photographyLayouts.length > 0) {
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.photographyLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.photographyLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.photographyLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.photographyLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.photographyLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postPhotographyPageData(this.photographyLayouts)
      .subscribe((data) => {
        if (data.message === 'success') {
          ...
          this.msgService.errorToaster(
            'Please add Photography studio image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
