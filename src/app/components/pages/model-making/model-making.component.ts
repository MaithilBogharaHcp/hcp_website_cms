...
@Component({
  selector: 'app-model-making',
  standalone: true,
  templateUrl: './model-making.component.html',
  styleUrl: './model-making.component.css',
  imports: [
    CmsTopbarComponent,
    TopbarComponent,
    NgxSpinnerModule,
...
export class ModelMakingComponent {
  isLoading = true;
  imageUrl = environment.image_url_server;
  modelmakingLayouts!: any;
  others: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  modelmakingDescription: any;
  ...
  fetchData() {
   this.service.getModelMaking().subscribe(
      (data) => {
        this.modelmakingLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      ...
      if (this.modelmakingLayouts.length > 0) {
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.modelmakingLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.modelmakingLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.modelmakingLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.modelmakingLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.modelmakingLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postModelMakingPageData(this.modelmakingLayouts)
      .subscribe((data) => {
        if (data.message === 'success') {
          ...
          this.msgService.errorToaster(
            'Please add Model Making studio image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
