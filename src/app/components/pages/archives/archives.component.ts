...
export class ArchivesComponent implements OnInit {
  ...
  fetchData() {
    this.service.getArchives().subscribe(
      (data) => {
        this.archivesLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      ...
      if (this.archivesLayouts.length > 0) {
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.archivesLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.archivesLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.archivesLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.archivesLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.archivesLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.service
      .postArchivesPageData(this.archivesLayouts)
      .subscribe((data) => {
       ...
       this.msgService.successToaster('Uploaded successfully');
          ...
          this.msgService.errorToaster(
            'Please add archives image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
