...
export class VisualizationComponent {
  ...
  fetchData() {
    this.service.getVisualization().subscribe(
      (data) => {
        this.visualizationLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      if (layoutId === '32') {
        this.visualizationLayouts.push({
          id: ``,
          title: '',
          description: '',
      ...
      if (this.visualizationLayouts.length > 0) {
  ...
  onLayoutReplaceSelected(event: any) {
    if (this.selectedLayoutToReplace) {
      ...
      if (selectedLayout.layout_id === '32') {
        this.visualizationLayouts[index] = {
          id: id,
          title: '',
          description: '',
          layoutid: selectedLayout.layout_id,
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.visualizationLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.visualizationLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.visualizationLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.visualizationLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.visualizationLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postVisualizationPageData(this.visualizationLayouts)
      .subscribe((data) => {
