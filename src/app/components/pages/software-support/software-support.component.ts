...
@Component({
  ...
  styleUrl: './software-support.component.css',
  imports: [
    CmsTopbarComponent,
    TopbarComponent,
    NgxSpinnerModule,
...
export class SoftwareSupportComponent {
  isLoading = true;
  imageUrl = environment.image_url_server;
  softwareSupportLayouts!: any;
  others: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  softwareSupportDescription: any;
  ...
  fetchData() {
    this.service.getsoftwareSupport().subscribe(
      (data) => {
        this.softwareSupportLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      if (layoutId === '32') {
        this.softwareSupportLayouts.push({
          id: ``,
          title: '',
          description: '',
      ...
      if (this.softwareSupportLayouts.length > 0) {
  ...
  onLayoutReplaceSelected(event: any) {
    if (this.selectedLayoutToReplace) {
      ...
      if (selectedLayout.layout_id === '32') {
        this.softwareSupportLayouts[index] = {
          id: id,
          title: '',
          description: '',
          layoutid: selectedLayout.layout_id,
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.softwareSupportLayouts.forEach((project: any, projectIndex: any) => {
  ...
  onFilesSelected(files: File | File[], index: number, layoutId: string) {
    ...
    fileArray.forEach((file, fileIndex) => {
      reader.onload = () => {
        ...
        this.softwareSupportLayouts.forEach(
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.softwareSupportLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.softwareSupportLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.softwareSupportLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.softwareSupportLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postsoftwareSupportPageData(this.softwareSupportLayouts)
      .subscribe((data) => {
