...
@Component({
  ...
  styleUrl: './conservation.component.css',
  imports: [
    CmsTopbarComponent,
    TopbarComponent,
    NgxSpinnerModule,
...
export class ConservationComponent {
  ...
  conservationLayouts!: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  others: any;
  conservationDescription: any;
  ...
  fetchData() {
    this.service.getConservation().subscribe(
      (data) => {
        this.conservationLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      ...
      if (this.conservationLayouts.length > 0) {
  ...
  onLayoutReplaceSelected(event: any) {
    if (this.selectedLayoutToReplace) {
      ...
      if (selectedLayout.layout_id === '32') {
        this.conservationLayouts[index] = {
          id: id,
          title: '',
          description: '',
          layoutid: selectedLayout.layout_id,
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.conservationLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.conservationLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.conservationLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.conservationLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.conservationLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.service
      .postConservationPageData(this.conservationLayouts)
      .subscribe((data) => {
       ...
       this.msgService.successToaster('Uploaded successfully');
          ...
          this.msgService.errorToaster(
            'Please add conservation image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
