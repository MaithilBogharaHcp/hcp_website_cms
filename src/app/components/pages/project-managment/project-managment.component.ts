...
@Component({
  ...
  styleUrl: './project-managment.component.css',
  imports: [
    CmsTopbarComponent,
    TopbarComponent,
    NgxSpinnerModule,
...
export class ProjectManagmentComponent {
  isLoading = true;
  imageUrl = environment.image_url_server;
  projectmanagementLayouts!: any;
  others: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  projectmanagementDescription: any;
  ...
  fetchData() {
    this.service.getProjectManagment().subscribe(
      (data) => {
        this.projectmanagementLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      if (layoutId === '32') {
        this.projectmanagementLayouts.push({
          id: ``,
          title: '',
          description: '',
      ...
      if (this.projectmanagementLayouts.length > 0) {
  ...
  onLayoutReplaceSelected(event: any) {
    if (this.selectedLayoutToReplace) {
      ...
      if (selectedLayout.layout_id === '32') {
        this.projectmanagementLayouts[index] = {
          id: id,
          title: '',
          description: '',
          layoutid: selectedLayout.layout_id,
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.projectmanagementLayouts.forEach(
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.projectmanagementLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.projectmanagementLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.projectmanagementLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.projectmanagementLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postProjectManagmentPageData(this.projectmanagementLayouts)
      .subscribe((data) => {
        if (data.message === 'success') {
          ...
          this.msgService.errorToaster(
            'Please add Project Managment studio image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
