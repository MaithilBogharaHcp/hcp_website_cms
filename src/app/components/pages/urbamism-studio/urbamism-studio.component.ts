...
@Component({
  ...
  templateUrl: './urbamism-studio.component.html',
  styleUrl: './urbamism-studio.component.css',
})
export class UrbamismStudioComponent {
  isLoading = true;
  imageUrl = environment.image_url_server;
  urbanismStudioLayouts!: any;
  isSideBar: boolean = false;
  isEdit: boolean = true; //Mode
  urbanismStudioDescription: any;
  ...
  fetchData() {
    this.service.getUrbanismStudioData().subscribe(
      (data) => {
        this.urbanismStudioLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      if (layoutId === '32') {
        this.urbanismStudioLayouts.push({
          id: ``,
          title: '',
          description: '',
      ...
      if (this.urbanismStudioLayouts.length > 0) {
  ...
  onLayoutReplaceSelected(event: any) {
    if (this.selectedLayoutToReplace) {
      ...
      if (selectedLayout.layout_id === '32') {
        this.urbanismStudioLayouts[index] = {
          id: id,
          title: '',
          description: '',
          layoutid: selectedLayout.layout_id,
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.urbanismStudioLayouts.forEach((project: any, projectIndex: any) => {
  ...
  onFilesSelected(files: File | File[], index: number, layoutId: string) {
    ...
    fileArray.forEach((file, fileIndex) => {
      reader.onload = () => {
        ...
        this.urbanismStudioLayouts.forEach(
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.urbanismStudioLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.urbanismStudioLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.urbanismStudioLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.urbanismStudioLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postUrStudioPageData(this.urbanismStudioLayouts)
      .subscribe((data) => {
       ...
       this.msgService.successToaster('Uploaded successfully');
          ...
          this.msgService.errorToaster(
            'Please add urbanism studio image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
