...
export class LandscapeComponent {
  ...
  fetchData() {
    this.service.getLandscap().subscribe(
      (data) => {
        this.landscapLayouts = data.data;
  ...
  onLayoutSelected(layoutId: any) {
    ...
    if (selectedLayout) {
      ...
      if (this.landscapLayouts.length > 0) {
  ...
  onFilesSelectedLayout(
    ...
    reader.onload = () => {
      ...
      this.landscapLayouts.forEach((project: any, projectIndex: any) => {
  ...
  openEditLayoutDialog(index?: any) {
    const id = this.landscapLayouts[index].id;
  ...
  changeTitle(event: any) {
    this.hasChanges = true;
    this.landscapLayouts[1].title = event;
  }

  changeDescription(event: any) {
    this.hasChanges = true;
    this.landscapLayouts[1].description = event.replace(/Â/g, '');
  }
  ...
  onLayoutDelete() {
    ...
    this.spinner.show();
    this.landscapLayouts.splice(index, 1);
    this.hasChanges = true;
    this.spinner.hide();
    this.isVisible = false;
  }
  ...
  onPublish() {
    ...
    this.spinner.show();
    this.service
      .postLandscapPageData(this.landscapLayouts)
      .subscribe((data) => {
        if (data.message === 'success') {
          ...
          this.msgService.errorToaster(
            'Please add landscap studio image or name.'
          );
          this.fetchData();
          this.spinner.hide();
        }
